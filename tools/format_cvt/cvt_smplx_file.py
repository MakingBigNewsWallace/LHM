...
def rewrite_smplx_json(file_path):
    #glob the json files in the folder, and rewrite the body pose to be LHM INIT
    import math
    smplx_json_files = glob(osp.join(file_path, '*.json'))
    smplx_json_files=sorted(smplx_json_files,key=lambda x:int(x.split('/')[-1][:-5]))
    for i in tqdm(range(len(smplx_json_files))):
        with open(smplx_json_files[i],'r') as f:
            smplx_json = json.load(f)
        # print(type(smplx_json["body_pose"][1]))
        smplx_json["body_pose"][0] = [0., 0., math.pi / 9]
        smplx_json["body_pose"][1] = [0., 0., -math.pi / 9]

        # write back to the json file
        with open(smplx_json_files[i],'w') as f:
            json.dump(smplx_json,f)
    
    print("rewritten smplx json files saved to ",file_path)
...
def main():
    ...
    rewrite_smplx_json("/home/wenbo/LHM/train_data/motion_video/LHM_neutral_pose/smplx_params")
